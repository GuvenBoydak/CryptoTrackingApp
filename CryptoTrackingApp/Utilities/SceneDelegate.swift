...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        window.rootViewController =  user != nil ? createTabBarController(rootViewController: TabBarViewController()) : LoginViewController()
        window.makeKeyAndVisible()
        self.window = window
        updateInterfaceStyle()
    }
    private func createTabBarController(rootViewController: UITabBarController)-> UITabBarController{
        let appearance = UITabBarAppearance()
        appearance.configureWithDefaultBackground()
        appearance.backgroundColor = .tertiarySystemGroupedBackground
